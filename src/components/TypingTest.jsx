  ...
  return (
    <motion.div className="min-h-svh" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="max-w-xl mx-auto px-4 py-6">
        ...
        <p className="text-sm text-ink-muted font-body mb-5 text-center">
        ...
        {/* Sliders */}
        <div className="bg-white rounded-2xl p-4 mb-5 shadow-sm ring-1 ring-black/5 space-y-3">
          {SLIDERS.map((s) => (
            <div key={s.key}>
              ...
              />
              <p className="text-[10px] text-ink-muted/50 font-body mt-0.5">{s.hint}</p>
            </div>
          ))}
        </div>

        {/* Keypress indicators */}
        <div className="flex justify-center gap-1 mb-3 min-h-[32px] flex-wrap items-end">
        ...
        {/* Typing area */}
        <div className="relative bg-white rounded-2xl p-4 shadow-sm ring-1 ring-black/5 hover:ring-key-300/20 transition-all min-h-[140px] mb-5">
          <textarea
            ref={textareaRef}
            value={text}
